...
class FeatureSelection():
    def select_features():
        data = np.genfromtxt('Dataset_Numbers_Only.csv',
                             delimiter=',', dtype=float, skip_header=1)
        features = np.genfromtxt(
            'Dataset_Features.csv', delimiter=',', dtype=str)

        award_share = features[51]  # this is the column titled 'award_share'
        seasons = features[0]
        ...
        award_share_data = imputed_data[:, 42]
        # seasons
